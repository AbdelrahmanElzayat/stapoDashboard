...
import HeaderInvoicesTable from "./HeaderInvoicesTable";
import { InvoicesType } from "@/utils/types/HeaderInfoType";
...
const InvoicesTable = <T extends InvoicesType>({
  data,
  label,
}: ReqTableProps<T>) => {
  ...
  const columns: TableColumn<InvoicesType>[] = [
    {
      name: "الكود",
  ...
  return (
    <div className="w-full rounded-[14px] mt-4 border-[1px] border-[#e6e6e6] mx-auto p-4 text-right">
      <HeaderInvoicesTable label={label} />
